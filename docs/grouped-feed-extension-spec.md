...
- 近期范围预设：`7天内 / 14天内 / 30天内` 三个内建选项，默认 `7天内`。
- 分组“上次看到”时间点：用户通过“上次看到这里”手动写入的自定义时间点；存在时优先于近期范围预设。
- 有效已阅基线：某作者在某分组下用于红点和“近期投稿”判定的最终基线。优先取作者级已阅时间点，否则取分组“上次看到”时间点，若不存在再取分组近期范围预设。
...
- “近期”的判定统一来自同一套分组级设置：
  - 预设范围：`7天内 / 14天内 / 30天内`，默认 `7天内`；
  - 自定义时间点：分组“上次看到”时间点；若存在则覆盖预设。
- 作者级已阅时间点优先级高于分组级设置；该优先级在三种模式下保持一致。

#### 4.4.1 时间流模式
- 模式目标：按时间顺序查看“近期投稿”。
- 时间流与“近期投稿”共享同一组分组级近期设置：
  - 近期范围预设：`7天内 / 14天内 / 30天内`；
  - 分组“上次看到”时间点（若存在则覆盖预设）。
- 时间流额外提供一个临时展示选项：`全部`。
  - `全部` 只影响当前时间流列表展示；
  - `全部` 不写入分组持久化状态；
...
- 当存在分组“上次看到”时间点时，该时间点覆盖当前 `7/14/30天内` 预设，成为时间流与“近期投稿”共享的有效基线。
...
- 点击后撤销最近一次由“上次看到这里”写入的分组级自定义时间点：
  ...
  - 若撤销后已无自定义时间点，则恢复当前保存的 `7/14/30天内` 预设。
...
- 过滤选项语义：
  ...
  - 不写入分组近期基线（`savedReadMarkTs` / `savedRecentPresetKey`）。
...
- 分组级未读计算：
  ...
  - 其余作者按分组有效已阅基线计算：优先使用分组“上次看到”时间点；若不存在，则使用分组当前 `7/14/30天内` 预设；
...
- `defaultReadMarkDays`：分组默认近期范围预设，默认 `7`，并作为时间流“至少展示天数”下界。
...
type RecentPresetKey = 'd7' | 'd14' | 'd30';
...
interface GroupRuntimeState {
  ...
  savedMode?: ViewMode;
  savedRecentPresetKey?: RecentPresetKey; // default d7，时间流/近期投稿共享
  savedReadMarkTs?: number; // 最新的分组“上次看到”时间点；存在时覆盖近期预设
  savedAllPostsFilter?: AllPostsFilterKey; // 不参与 unread，只影响“全部投稿”展示
  savedByAuthorSortByLatest?: boolean; // “近期投稿” default true
}
...
26. 时间流和“近期投稿”共享同一组近期设置：默认 `7天内`，可切换 `14天内 / 30天内`，且分组“上次看到”时间点存在时优先覆盖预设。
