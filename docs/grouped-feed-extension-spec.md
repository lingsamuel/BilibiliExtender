...
- 作者模式：按作者分段展示，支持作者内分页查看投稿。
- 已阅时间点：用户在分组内选择的“已阅时间点”（可为具体时间、默认已阅天数或“全部”）。
- 概览过滤：仅影响当前展示、不参与 unread 计算的快速筛选（如 14/30 天、10/30 条）。
...
#### 4.4.3 已阅下拉扩展（追踪 vs 概览）

- 下拉菜单在“时间流 / 按作者”模式均可显示以下概览选项：
  - `14天内`
  - `30天内`
  - `10条`
  - `30条`
...
8. 作者模式在“全部（t:0）”筛选下支持作者内分页，分页大小使用投稿 API 的 `ps` 参数值。
9. 红点按“追踪基线”生效：概览过滤（14/30 天、10/30 条）不影响分组红点和 Header unread。
