...
29. 时间流“上次看到这里”支持任意已阅时间点，不再仅支持天数分割；分割线显示不截断后方内容。
30. 时间流在任意已阅时间下仍保持“至少 `defaultReadMarkDays` 天（默认 7 天）”展示窗口，避免切换已阅点导致内容突变。
31. 任意接口/操作失败时，前台通过通知弹框提示错误，不再把主内容区替换为错误文本。
32. 关注/取关调用 `x/relation/modify` 时必须携带 `extend_content` 字段；其他扩展参数非强制。
33. 提供可调用的视频点赞与投币 API（消息链路贯通）。
34. 每位作者标题行提供“一键点赞”按钮，可对当前可见视频批量点赞，并通过调度器串行执行。
35. VideoCard 封面左下角支持点赞拇指角标：未点赞不显示，已点赞显示粉色图标。
