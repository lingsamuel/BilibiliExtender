...
- 仅当已阅筛选为追踪选项 `全部（t:0）` 时启用作者内分页。
- 当处于概览筛选（14/30 天、10/30 条）或非 `t:0` 的追踪筛选时，不显示分页控件，沿用现有筛选结果直出。
...
3. 命中边界时（包括缺失第二页场景），向 Burst 队列**队首**插入该作者“下一页补页任务”，并等待该任务执行完成后重构结果。
...
8. 未观看模式在“全部（t:0）”筛选下支持作者内分页，分页大小使用投稿 API 的 `ps` 参数值，且分页器整体水平居中。
...
17. 时间流“加载更多”按钮仅在仍有更多内容时显示；命中作者缓存边界时可触发 Burst 队首补页并等待，失败时给出错误提示并 best-effort 返回。
...
26. 概览模式下作者级已阅仅影响标记线展示，不触发“额外 N 条”截断。
27. 作者级已阅标记线显示在两张视频卡片之间的中线位置。
28. 只要处于时间流视图，命中作者级逻辑已阅的视频卡片默认半透明，hover 恢复不透明，并带过渡动画。
