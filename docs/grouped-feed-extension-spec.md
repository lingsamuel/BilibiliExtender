...
- 时间流展示必须保留“至少当前近期范围预设对应窗口”的可见内容（默认 `7天内`）。
...
- `defaultReadMarkDays`：分组默认近期范围预设，仅允许 `7 / 14 / 30`，默认 `7`，并作为时间流“至少展示窗口”的基准。
...
31. 时间流在任意已阅时间下仍保持“至少当前近期范围预设（默认 `7天内`）”的展示窗口，避免切换已阅点导致内容突变。
